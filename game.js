...
class MidnightCafeMystery {
    constructor(containerId) {
        ...
        this.renderCaseSelect = this.renderCaseSelect.bind(this);
        this.restartGame = this.restartGame.bind(this);
        this.handleChoice = this.handleChoice.bind(this);
        this.skipText = this.skipText.bind(this);
    ...
    renderCaseSelect() {
        ...
        CASES.forEach(c => {
            ...
            `;
            btn.addEventListener('click', () => this.selectCase(c.id));
    ...
    selectCase(caseId) {
        // Map caseId to starting scene
        const startScene = {
            'sakurairo': 'sakurairo_intro',
            'hoshi': 'hoshi_intro',
            '3am': '3am_intro'
        }[caseId];

        this.state.currentScene = startScene;
        this.renderScene();
    }
    ...
    getSceneStyle(sceneType) {
        switch (sceneType) {
            case 'exterior': return 'scene-exterior';
            case 'memory': return 'scene-memory';
            case 'deduction': return 'scene-deduction';
            default: return 'scene-default';
        }
    }

    renderScene() {
        const sceneData = STORIES[this.state.currentScene];

        // Update background class
        this.container.className = `game-container ${this.getSceneStyle(sceneData.scene)}`;

        // Clear container but keep rain
        const rain = this.container.querySelector('.rain-container');
        this.container.innerHTML = '';
        if (rain) this.container.appendChild(rain);
        else this.createRainEffect();

        // Header
        const header = document.createElement('header');
        header.className = 'game-header';
        header.innerHTML = `
            <div class="game-logo" style="cursor:pointer">CAFE MEMORIA</div>
        `;
        header.querySelector('.game-logo').addEventListener('click', this.restartGame);
        ...
        main.addEventListener('click', this.skipText);
        ...
        textArea.style.boxShadow = '0 5px 15px rgba(0,0,0,0.3)';
        ...
        choicesContainer.style.gap = '0.8rem';
        ...
        // Start Typing
        this.typeWriter(sceneData.text, p, choicesContainer, sceneData);
    }

    typeWriter(text, element, choicesContainer, sceneData) {
        ...
        this.typingTimer = setInterval(() => {
            if (index < text.length) {
                ...
                this.finishTyping(choicesContainer, sceneData);
            }
        }, 30);
    }
    ...
    skipText() {
        if (this.state.isTyping) {
            ...
            const sceneData = STORIES[this.state.currentScene];
            const textArea = this.container.querySelector('.game-text');
            textArea.textContent = sceneData.text;
            this.finishTyping(this.container.querySelector('.game-choices'), sceneData);
        }
    }

    finishTyping(choicesContainer, sceneData) {
        ...
        if (sceneData.choices && sceneData.choices.length > 0) {
            sceneData.choices.forEach(choice => {
                ...
                btn.innerHTML = `<span class="arrow">▶</span> ${choice.text}`;
        ...
        if (sceneData.ending) {
            ...
            if (sceneData.sunoUrl) {
                ...
                embedContainer.style.marginBottom = '20px';
            ...
            restartBtn.style.maxWidth = '300px';
            restartBtn.style.margin = '2rem auto 0';
            restartBtn.style.background = 'transparent';
            restartBtn.style.border = '1px solid rgba(255, 255, 255, 0.5)';
    ...
    handleChoice(nextSceneId) {
        this.state.currentScene = nextSceneId;
        this.renderScene();
    }
}

...
- pattern: 柄（無地 / ストライプ / ボーダー / チェック / 花柄 / ドット / ペイズリー など）。無地の場合は「無地」と書く
...
- searchKeyword: ZOZOTOWNや楽天ファッションで検索すると「似た商品」が上位に出る日本語キーワード。
  構成: 「色 + 柄(無地以外) + 素材/質感 + シルエット/ディテール + カテゴリ名」の順。
...
- keywordVariants: 検索キーワードを具体的→汎用の順で3案。
  [0] 最も具体的（searchKeywordと同じ。色+柄+素材+シルエット+カテゴリ）
  [1] 中間（色+シルエットorディテール+カテゴリ。例「くすみブルー オーバーサイズ シャツ」）
  [2] 汎用（色+カテゴリのみ。例「ブルー シャツ」）
  ECサイトで[0]がヒットしない場合に[1][2]へ切り替えて検索する想定。
- searchKeywordShort: keywordVariants[2] と同じ内容（2〜3語の「色 + カテゴリ」）
- boundingBox: 画像内でそのアイテムが写っている領域を、画像サイズを 1.0 とした正規化座標で指定。
  x = 左端の位置（0.0〜1.0）、y = 上端の位置（0.0〜1.0）、w = 幅の割合、h = 高さの割合。
  そのアイテムだけがなるべく大きく写るよう、正確にタイトに囲むこと（後で画像検索の切り抜きに使用する）。
  例: トップスのみ → {"x":0.25,"y":0.18,"w":0.5,"h":0.32} / シューズ → {"x":0.3,"y":0.85,"w":0.4,"h":0.13}
    ...
    items: {
      ...
      items: {
        ...
        properties: {
          ...
          searchKeyword: { type: "string" },
          keywordVariants: { type: "array", items: { type: "string" } },
